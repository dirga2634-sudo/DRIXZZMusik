...
- **Error aneh yang gak nyambung sama kode (misalnya tiba-tiba banyak `attribute res-auto:... not found` padahal sebelumnya build sempat lolos linking tahap itu)** — sempat dicurigai cache Gradle lintas-run yang basi, sudah dimatikan (`cache-disabled: true` di step "Set up Gradle") tapi errornya tetap muncul lagi. Ternyata lebih mungkin AGP 9.3.0-nya sendiri yang masih terlalu baru (AGP 9.0 itu "major release" dengan banyak perubahan perilaku internal) dan belum cukup teruji dengan kombinasi appcompat/material versi tertentu. Project ini sekarang pakai **AGP 8.7.0 + Gradle 8.9** — kombinasi lama yang sudah dipakai jutaan project sejak akhir 2024, jauh lebih teruji, tetap mendukung penuh compileSdk 35 (Android 15). `cache-disabled: true` masih dibiarkan aktif untuk sementara sampai beberapa run berturut-turut sukses.
